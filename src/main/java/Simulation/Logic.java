...
public class Logic {
    ...
    private void initialize() {
        ...
        for (int i = 0; i < width * height; i++) {
            ...
            if (rngNum < 90.0) {
                cellList.add(new TissueCell(Calculator.coordFromIndex(i).getX(),Calculator.coordFromIndex(i).getY()));
            }
            else if (rngNum >= 90.0 && rngNum < 95.0) {
                cellList.add(new ImmuneCell(Calculator.coordFromIndex(i).getX(),Calculator.coordFromIndex(i).getY()));
            } else {
                cellList.add(new CancerCell(Calculator.coordFromIndex(i).getX(),Calculator.coordFromIndex(i).getY()));
            }
        }
    }
